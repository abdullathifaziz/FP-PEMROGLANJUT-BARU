...
namespace FP_PEMROGLANJUT.Model
{
    class TransaksiModel
    {
        ...
        public DataSet SelectDataGrid(string search)
        {
            ...
            if (search == "")
            {
                dsSelectDataGrid = temp.CustomSelectJoin("transaksi FULL JOIN produk ON produk.id_produk = transaksi.id_produk FULL JOIN tagihan ON tagihan.id_tagihan = transaksi.id_tagihan FULL JOIN pembeli ON pembeli.id_transaksi = transaksi.id_transaksi FULL JOIN pembayaran ON pembayaran.id_transaksi = transaksi.id_transaksi", null);
            }
            else
            {
                ...
                dsSelectDataGrid = temp.CustomSelectJoin("transaksi FULL JOIN produk ON produk.id_produk = transaksi.id_produk FULL JOIN tagihan ON tagihan.id_tagihan = transaksi.id_tagihan FULL JOIN pembeli ON pembeli.id_transaksi = transaksi.id_transaksi FULL JOIN pembayaran ON pembayaran.id_transaksi = transaksi.id_transaksi", kondisi);
            }
